...
.pull-right[
```{r text-data-2, eval=T, echo=F, ref.label='st'}
...
.pull-right[
```{r text-data-3, eval=T, echo = F, ref.label="wc", message=F}
```
]
...
.pull-right[
```{r text-data-4, eval=T, echo = F, ref.label="wc2", message=F}
```
]
...
.pull-right[
```{r text-data-6, eval=T, echo = F, ref.label="tf"}
```
]

---

```{r text-data-7, include=F}
...
.pull-right[
```{r text-data-8, eval=T, echo = F, ref.label="tf2"}
```
]
---
layout: true
...
```{r text-data-9, include=F, cache=TRUE}
library(dplyr)
library(tidytext)
library(janeaustenr)
...
.pull-right[
```{r text-data-10, eval=T, echo=F, ref.label='net1'}
...
.pull-right[
```{r text-data-11, eval=T, echo = F, ref.label="net2"}
```
]
---

```{r text-data-12, include=F}
library(dplyr)
library(tidyr)
library(tidytext)
library(ggplot2)
...
library(gutenbergr)
kjv <- gutenberg_download(10, mirror = "https://gutenberg.pglaf.org/")
library(stringr)
...
.pull-right[
```{r text-data-13, eval=T, echo = F, ref.label="kjb"}
```
]
...
```{r text-data-14, include=F, cache=TRUE}
library(jsonlite)
...
.pull-right[
```{r text-data-15, eval=T, echo = F, ref.label="commonwords"}
```
]
...
.pull-right[
```{r text-data-16, eval=T, echo = F, ref.label="connect2"}
```
]
...
.pull-right[
```{r text-data-17, eval=T, echo = F, ref.label="connect3"}
```
]
...
* [Text Analytics for every line in The Office](https://www.jennadallen.com/post/text-analytics-every-line-from-the-office/)
