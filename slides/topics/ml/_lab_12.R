...
plotcorr(d_ord, mar = c(1, 1, 1, 1), col = my_colors[d_ord * 50 + 50]) # base plotting
...
iris %>%
  ...
  select(-species) %>%
  dist() %>%
  hclust(method = "single") -> iris_cl # can play around with 'single', 'average', median', 'centroid'
...
d <- as.data.frame(Rtsne::Rtsne(t(df), perplex = 10)$Y) %>%
